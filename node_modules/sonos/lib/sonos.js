...
Sonos.prototype.searchMusicLibrary = function (searchType, searchTerm, options, callback) {
  ...
  if (!opensearch) searches = searches.concat((searches.slice(-1) === ':' ? '' : ':') + searchTerm)
  ...
  return contentDirectory.Browse(opts, function (err, data) {
    ...
    return (new xml2js.Parser()).parseString(data.Result, function (err, didl) {
      ...
      _.each(resultcontainer, function (item) {
        ...
        var res = {};
	      if (util.isArray(item.res) && item.res[0] && item.res[0].$) {
					res = item.res[0].$;
	      }
        items.push({
          ...
          'duration': res.duration ?
            (parseInt(res.duration.split(':')[0], 10) * 60 * 60) +
            (parseInt(res.duration.split(':')[1], 10) * 60) +
            parseInt(res.duration.split(':')[2], 10) :
            0,
          'protocolInfo': res.protocolInfo ? res.protocolInfo : null,
